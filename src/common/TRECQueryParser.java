...
public class TRECQueryParser extends DefaultHandler {
    ...
    public Query getAnalyzedQueryNEWS(Report r,IndexReader ireader) throws Exception {
		...
		mlt.setBoost(true);
		String[] sarr = {"content","title"};
		mlt.setFieldNames(sarr);
		
		
        Query luceneQuery = mlt.like(r.index);
        */
    	// rahul 
    	String s = r.content;
    	if(Constants.FIELD_TO_EXTRACT_QUERY_FROM.equals("title"))
    		s = r.title;
    	String titleContentQuery = TRECQuery.queryFieldAnalyze(analyzer,s);
    	
        Query luceneQuery = queryParser.parse(titleContentQuery,Constants.FIELD_TO_ISSUE_QUERY_AGAINST);
        //Query lq2 = queryParser.parse(titleContentQuery, "title");
        
        System.out.println("Original Query: "+luceneQuery.toString());
        //trecQuery.luceneQuery = luceneQuery;
