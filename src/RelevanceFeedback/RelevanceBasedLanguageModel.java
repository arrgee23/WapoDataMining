...
import common.Topic;
import common.TopicParser;
import common.LuceneQuery;
...
public class RelevanceBasedLanguageModel {
    ...
    public RelevanceBasedLanguageModel(Properties prop) throws IOException, Exception {
        ...
        indexReader = DirectoryReader.open(FSDirectory.open(Paths.get(prop.getProperty("indexPath"))));
...
//        FileWriter baselineRes = new FileWriter(resPath+".baseline");
        ...
        List <Topic> lt = TopicParser.getAllTopics(Constants.TOPIC_XML_SOURCE);
        
        for(Topic t : lt) {
        	// search for the doc with id to get query document
        	//System.out.println(t.docid);
        	Integer id = new Integer(0);
        	Report r = LuceneQuery.getReportWithID(t.docid, indexSearcher,id);
        	Query luceneQuery = trecQueryparser.getAnalyzedQueryNEWS(r,indexReader);
        	
        	TRECQuery tq = new TRECQuery();
        	tq.fieldToSearch = Constants.FIELD_TO_ISSUE_QUERY_AGAINST;
        	tq.luceneQuery = luceneQuery;
        	tq.qid = t.number.substring(9, t.number.length()-1);;//r.getId();
        	
        	tq.qtitle = luceneQuery.toString(Constants.FIELD_TO_ISSUE_QUERY_AGAINST);//r.getTitle()+r.getContent();
        	//System.out.println("***"+tq.qtitle);
        	//tq.qdesc = r.getTitle();
        	//tq.qnarr = r.getTitle();
        ...
        for (TRECQuery query : queries) {
            ...
            {
                indexSearcher.search(query.luceneQuery, collector);
            ...
            FileWriter baselineRes = new FileWriter(resPath+".baseline", true);

            resBuffer = new StringBuffer();
            resBuffer = CommonMethods.writeTrecResFileFormat(query.qid, topDocs.scoreDocs, indexSearcher, runName);
            ...
            if(true) {
                ...
                rlm.setFeedbackStats(topDocs, query.luceneQuery.toString(Constants.FIELD_TO_ISSUE_QUERY_AGAINST).split(" "), this);
                /**
    ...
    public static void main(String[] args) throws IOException, Exception {
        ...
        args = new String[1];
        args[0] = "/home/i3/git/WapoDataMining/rblm.D-20.T-70.properties";
