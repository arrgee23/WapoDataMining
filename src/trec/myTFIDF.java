...
public final class myTFIDF {
	 ...
	 */
	public myTFIDF(IndexReader ir) {
		this(ir, new ClassicSimilarity());
	}

	public myTFIDF(IndexReader ir, TFIDFSimilarity sim) {
		this.ir = ir;
		this.similarity = sim;
	}
	 ...
	 *
	 * @param maxFreq the maximum count of documents that a term may appear in to be
	 ...
	 *
	 * @param maxPercentage the maximum percentage of documents (0-100) that a term
	 *                      may appear in to be still considered relevant.
	 */
	 ...
	 *
	 * @param fieldNames the field names that will be used when generating the 'More
	 *                   Like This' query.
	 */
	 ...
	 * query.
	 *
	 * @param maxQueryTerms the maximum number of query terms that will be included
	 *                      in any generated query.
	 */
	 ...
	 */
	public Query createQuery(PriorityQueue<ScoreTerm> q) {
	 ...
	 *
	 * @param perFieldTermFrequencies a per field map of words keyed on the
	 *                                word(String) with Int objects as the values.
	 */
	public PriorityQueue<ScoreTerm> createQueue(Map<String, Map<String, Int>> perFieldTermFrequencies)
			throws IOException {
	...
	private float getBM25Score(String word, int tf, String fieldName, IndexReader ir, int docSize, float k1, float b) {
		// compute bm 25 score
		int docFreq;
		try {
			IndexSearcher is = new IndexSearcher(ir);
			CollectionStatistics cs = is.collectionStatistics(fieldName);
			
			// idf
			long docCount = cs.docCount();
			docFreq = ir.docFreq(new Term(fieldName, word));
			float idf = idf(docFreq, docCount);
			
			// tf
			float avgFieldLength = avgFieldLength(cs);
			
			float top = tf*(k1+1);
			float bottom = tf+k1*(1-b+b*(docSize/avgFieldLength));
	 ...
	 */
	public PriorityQueue<ScoreTerm> retrieveTerms(int docNum) throws IOException {
		...
		// return createQueueBM25(field2termFreqMap);
	}
	 ...
	 */
	private void addTermFrequencies(Map<String, Map<String, Int>> field2termFreqMap, Terms vector, String fieldName)
			throws IOException {
	 ...
	 * @param fieldName               Used by analyzer for any special per-field
	 *                                analysis
	 */
	...
	// class for storing each term along with score and field
