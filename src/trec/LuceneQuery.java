...
public class LuceneQuery {
	...
	public static Query myTitleContentQuery(Report r, IndexSearcher isearcher, IndexReader ireader) {
		...
		Query query = null;
		Query query2 = null;
		String queryStr = makeTitleContentQuery(r, isearcher, 2000);
		QueryParser parser2 = new QueryParser("title", analyzer);
		
		try {
			query = parser.parse(queryStr);
			query2 = parser2.parse(queryStr);
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		return mergeQueries(query,query2);
	}
			...
			for (int i = 0; i < hits.length; i++) {
				...
				// )
					...
					if (type != null) {
						...
						if (type.equals("Opinion") || type.equals("Opinios") || type.equals("Letters to the Editor")
								|| type.equals("The Post's View")) {
							continue;
						}
	...
	/*
	 * public static Query makeTopKTermQueryFromReportWrapper(int k,String
	 * fieldToExtractTermsForm, Report r,IndexReader ireader, IndexSearcher
	 * isearcher, int minTermFreq , String[] fieldsToIssueQueryOn) {
	 * if(fieldToExtractTermsForm.equals("contentner") ||
	 * fieldToExtractTermsForm.equals("titlener") )
	 * 
	 * return makeTopKNERTermQueryFromReport(k,fieldToExtractTermsForm,r,
	 * ireader,isearcher,minTermFreq,fieldsToIssueQueryOn); else return
	 * makeTopKTermQueryFromReport(k,fieldToExtractTermsForm,r,
	...
	public static Query makeTopKTermQueryFromReport(int k, String fieldToExtractTermsForm, Report r,
			IndexReader ireader, IndexSearcher isearcher, int minTermFreq, float lambda,
			String[] fieldsToIssueQueryOn,boolean isBoostOn) {
		...
		if (lucene_id != 0) {
			try {
				...
				while ((scoreTerm = pqContent.pop()) != null && count < k) {
					...
					/*
					 * costruct the query such that Search for the key word on
					 * given(fieldsToIssueQueryOn) set of fields
					 */
	...
	/*
	 * Merge two queries, the set of fields q1 and q2 is applied on must be disjoint
	 */
	...
	public static TopDocs getResultsDumbQuery(Report r, IndexSearcher isearcher,
			IndexReader ireader) {
		Query q = myTitleContentQuery(r, isearcher, ireader);
		
		TopDocs td1 = null;
		try {
			BooleanQuery.setMaxClauseCount(Integer.MAX_VALUE);
			td1 = isearcher.search(q, 100);
		} catch (IOException e) {
			e.printStackTrace();
		}
		return td1;
	}
	public static TopDocs getResultsTopTerms(Report r, IndexSearcher isearcher,
			IndexReader ireader) {
		
		String[] fieldsToIssueQueryOn = { "title", "content" };
		String[] fieldsToIssueQueryOn3 = { "title"};

		float lambda = (float) 0.8;
		//Query titleQuery = makeTopKTermQueryFromReport(Constants.TITLE_QUERY_LENGTH, "title", r, ireader, isearcher, 1,
			//	lambda, fieldsToIssueQueryOn3,true);

		Query contentQuery = makeTopKTermQueryFromReport(Constants.CONTENT_QUERY_LENGTH, "content", r, ireader,
				isearcher, 2, lambda, fieldsToIssueQueryOn,true);

		//Query content_title_query = mergeQueries(contentQuery, titleQuery);

		// float lambda = (float) 0.6;
		// Query contentQuery =
		// makeTopKTermQueryFromReport(Constants.CONTENT_QUERY_LENGTH, "content", r,
		// ireader,isearcher,2,lambda,fieldsToIssueQueryOn);

		// ner query
		String[] fieldsToIssueQueryOn2 = {"title", "titlener"};
		Query queryTitleNer = makeTopKTermQueryFromReport(Constants.TITLE_QUERY_LENGTH, "titlener", r,
		ireader,isearcher,1,1-lambda,fieldsToIssueQueryOn2,false);
		
		// merge content and ner query
		Query contentTitleNerQuery = mergeQueries(contentQuery,queryTitleNer);
		...
		// Query content_queryContentNer_title = mergeQueries(titleQuery,
		// content_queryContentNer);

		// Hits hits = is.search(query);
		// now the usual iteration thru 'hits' - the only thing to watch for is to make
		// sure
		// you ignore the doc if it matches your 'target' document, as it should be
		// similar to itself
		TopDocs td1 = null, td2 = null, td3 = null, td4 = null, td5 = null, td8=null,td6 = null, td7 = null;
		try {
			...
			// td7 = isearcher.search(content_queryContentNer_title, 100);
			td8 =  isearcher.search(contentTitleNerQuery,100);
		} catch (IOException e) {
			e.printStackTrace();
		}

		TopDocs tdMerged = null;
		//tdMerged = weightedMergeTopdoc(td1, td2, (float)0.1, false);
		tdMerged = td8;
	...
	/*
	 * Merge two topdocs with scores changed as t1[i].score =*lambda and
	 * t2[i].score=*(1-lambda) then merge two lists and rerank during merging if any
	 * one of them appears in both list, then they are added
	 * 
	 */
	public static TopDocs weightedMergeTopdoc(TopDocs t1, TopDocs t2, float lambda, boolean isNormalize) {
		if (t2 == null)
			return t1;
	...
	/*
	 * public static List<Tuple> getResultsContent(Report r, IndexSearcher
	 * isearcher) { // Parse a simple query that searches for "text": Analyzer
	 * analyzer = new EnglishAnalyzer(); // ..standard english word tokenization
	 * QueryParser parser = new QueryParser("content", analyzer);
	 * 
	 * Query query = null;
	 * 
	 * List<String> st = returnStringTokens(r.title); List<String> sl =
	 * returnStringTokens(r.content); StringBuffer sb = new StringBuffer();
	 * 
	 * int count = 0;
	 * 
	 * for (String s : st) { if (count > 1000) break; sb.append(s); sb.append(" ");
	 * 
	 * count++; }
	 * 
	 * for (String s : sl) { if (count > 1000) break;
	 * 
	 * sb.append(s); sb.append(" ");
	 * 
	 * count++; }
	 * 
	 * try { query = parser.parse(sb.toString()); } catch (Exception e) {
	 * e.printStackTrace(); }
	 * 
	 * ScoreDoc[] hits = null; try { hits = isearcher.search(query, 1000).scoreDocs;
	 * } catch (IOException e) { e.printStackTrace(); } // assertEquals(1,
	 * hits.length);
	 * 
	 * if (hits.length < 1) {
	 * System.out.println("Your query did not return any result..."); assert
	 * (false); }
	 * 
	 * LinkedList<Tuple> answers = new LinkedList<Tuple>(); try { int added = 0; for
	 * (int i = 0; i < hits.length; i++) {
	 * 
	 * if (added >= 100) break;
	 * 
	 * String id = isearcher.doc(hits[i].doc).get("id"); String dateStr =
	 * isearcher.doc(hits[i].doc).get("date");
	 * 
	 * @SuppressWarnings("deprecation") // Date resultReportDate = new
	 * Date(dateStr); // Date queryReportDate = new Date(r.date); long
	 * resultReportDate = 0; long queryReportDate = 0;
	 * 
	 * if (dateStr != null && r.date != null && !dateStr.equals("null") &&
	 * !r.date.equals("null")) { resultReportDate = Long.parseLong(dateStr);
	 * queryReportDate = Long.parseLong(r.date); } double score = hits[i].score;
	 * String codename = "arrgee1";
	 * 
	 * // if(queryReportDate>=resultReportDate){ answers.add(new Tuple(id, score,
	 * codename)); added++; // } }
	 * 
	...
	public static List<Tuple> getResultsContentSorted(Report r, IndexSearcher isearcher) {
		...
		try {
			...
			for (int i = 0; i < hits.length; i++) {
				...
				// “Opinion”, “Letters to the Editor”, or “The Post’s View” are eliminated
				// if()
				if (count < 100) {
					String id = isearcher.doc(hits[i].doc).get("id");
					double score = hits[i].score;
	...
	public static List<Tuple> getResultsWrapper(Report r, IndexSearcher isearcher, DirectoryReader ireader,
			ReportToVectors rvs) {
		TopDocs td = null;
		if (rvs != null) {
		...
		TopDocs dumbQuery  = getResultsDumbQuery(r, isearcher, ireader);
		tdMerged = dumbQuery;
		...
		try {
			...
			for (int i = 0; i < hits.length; i++) {
				...
				// convert string to date when filterinng to classes
				// TODO add Date field in report class
				if (dateStr != null && r.date != null && !r.date.equals("") && !dateStr.equals("")
						&& !dateStr.equals("null") && !r.date.equals("null")) {
					Instant result_instant = Instant.ofEpochSecond(Long.parseLong(dateStr));
					Instant query_instant = Instant.ofEpochSecond(Long.parseLong(r.date));
				...
				// allow only articles published on or before
				if (resultDate == null || queryDate == null ||
				// resultDate.after(queryDate) ||
						resultDate.before(queryDate)
						|| (resultDate.getDay() == queryDate.getDay() && resultDate.getMonth() == queryDate.getMonth()
								&& resultDate.getYear() == queryDate.getYear())) {
					String type = isearcher.doc(hits[i].doc).get("articleType");
					if (type != null) {
						// ignore articles with this type as specified
						if (type.equals("Opinion") || type.equals("Opinions") || type.equals("Letters to the Editor")
								|| type.equals("The Post's View")) {
							continue;
						}
					...
					/*
					 * ignore short length stories String content =
					 * isearcher.doc(hits[i].doc).get("content"); String[] words =
					 * content.split(" "); if(words.length<200) continue;
					 */
